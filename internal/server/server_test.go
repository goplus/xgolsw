...
func TestHandleMessageInitialization(t *testing.T) {
	...
	t.Run("CancelInitialize", func(t *testing.T) {
		...
		retryCall, err := jsonrpc2.NewCall(jsonrpc2.NewStringID("initialize-retry"), "initialize", InitializeParams{})
		require.NoError(t, err)
		require.NoError(t, server.HandleMessage(retryCall))

		retryResponse := replier.waitForResponse(retryCall.ID(), 5*time.Second)
		require.NotNil(t, retryResponse)
		require.NoError(t, retryResponse.Err())
