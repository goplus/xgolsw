...
const (
	// ResolvedCallExprKwargTargetUnknown identifies an unresolved kwarg target.
...
func ResolveFuncSignature(fun *gotypes.Func) (sig *gotypes.Signature, params *gotypes.Tuple) {
	...
	params = normalizedCallExprParams(fun, sig)
	return sig, params
}
...
func normalizedCallExprParams(fun *gotypes.Func, sig *gotypes.Signature) *gotypes.Tuple {
	...
	for i := range params.Len() - 1 {
		vars = append(vars, params.At(i+1))
	}
	return gotypes.NewTuple(vars...)
}

// resolvedCallExprArgType returns the expected argument type at paramIndex and
// unwraps variadic slices to their element type.
func resolvedCallExprArgType(sig *gotypes.Signature, params *gotypes.Tuple, paramIndex int) gotypes.Type {
...
func interfaceKwargMethodTarget(method *gotypes.Func, self *gotypes.Named) (ResolvedCallExprKwargTarget, bool) {
	...
	if sig.Variadic() {
		valueType = valueType.(*gotypes.Slice).Elem()
	}
	return ResolvedCallExprKwargTarget{
...
func LookupResolvedCallExprKwargTarget(kwarg *ResolvedCallExprKwarg, name string) *ResolvedCallExprKwargTarget {
	...
	case *gotypes.Interface:
		named, _ := paramType.(*gotypes.Named)
		if named == nil {
			if u.Empty() {
		...
		if target := lookupInterfaceKwargMethodTarget(u, named, name); target != nil {
			return target
		}
		if hasInterfaceKwargSet(u, named) {
			return &ResolvedCallExprKwargTarget{
...
// ListResolvedCallExprKwargTargets lists the named keyword targets exposed by
// the target parameter slot. Dynamic `Set`-style interface kwargs are not
// enumerated.
	...
	return func(yield func(ResolvedCallExprArg) bool) {
		...
		for i, arg := range expr.Args {
			...
			if !yield(ResolvedCallExprArg{
				...
				ExpectedType: resolvedCallExprArgType(sig, params, paramIndex),
			}) {
				return
			}
		}
